...
class ConfigGlasgow:
    k_n = 16  # KNN
    num_layers = 5  # Number of layers
    num_points = 65536  # Number of input points # todo check point
    num_classes = 4  # Number of valid classes
    sub_grid_size = 0.32  # preprocess_parameter

    batch_size = 4  # batch_size during training
    # val_batch_size = 8  # batch_size during validation and test
    val_batch_size = 16  # batch_size during validation and test for Shatin data
    train_steps = 500  # Number of steps per epochs
    val_steps = 100  # Number of validation steps per epoch
...
class DataProcessing:
    ...
    def get_dataset_root_path():
        ...
        # todo
        if user_name == 'qll3h' and host_name == 'ubdc-gpusys02':
            #root_path == '/home/qll3h/ql_project/LiDAR_Data/Dataset/'
            root_path = '//data'
        else:
    ...
    def get_num_class_from_label(labels, total_class):
        ...
        for idx, val in enumerate(val_list):
            num_pts_per_class[val] += counts[idx] # original
            # num_pts_per_class[idx] += counts[idx] # this line is  for our data
    ...
    def get_nuscenes_file_list(dataset_path):
        ...
        test_file_list = np.array([join(dataset_path, 'achived_test/test', 'velodyne', f) for f in
                                   np.sort(os.listdir(join(dataset_path, 'achived_test/test', 'velodyne')))])
    ...
    # ql
    def save_las2ply(self, cloud_name, xyz, attributes, labels, grid_size=0.1):
        print('Preparation of {:s}'.format(cloud_name))
        full_ply_path = join(self.original_pc_folder, cloud_name + '.ply')
        sub_ply_file = join(self.sub_pc_folder, cloud_name + '.ply')
        write_ply(full_ply_path, (xyz, attributes, labels),
                  ['x', 'y', 'z', 'intensity', 'numberofreturn', 'returnnumber', 'class'])
        # grid_sub_sampling to save memory and computation
        sub_xyz, sub_attributes, sub_labels = self.grid_sub_sampling(xyz, attributes, labels, grid_size)
        sub_labels = np.squeeze(sub_labels)
        write_ply(sub_ply_file, [sub_xyz, sub_attributes, sub_labels], ['x', 'y', 'z', 'intensity', 'numberofreturn', 'returnnumber', 'class'])
