...
class ModelTester:
    ...
    def test(self, model, dataset, gen_pseudo=None, num_votes=100):
        ...
        test_path = join('checked_Script/test', saving_path.split('/')[-1])
                ...
                if last_min + 1 < new_min:
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_val):
                            ...
                            name = dataset.input_names['validation'][i_test] + '_o.ply'
                            write_ply(join(test_path, 'val_preds', name), [preds_o], ['pred'])
