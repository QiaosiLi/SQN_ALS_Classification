...
class Shatin:
    ...
    def load_sub_sampled_clouds(self):
        ...
        for i, file_path in enumerate(self.train_files):
            ...
            # ann_attributes = np.vstack((data['intensity'], np.zeros_like(data['numberofreturn']), np.zeros_like(data['returnnumber']))).T
            # ann_attributes = np.zeros_like(ann_attributes)
            # ann_attributes = data['intensity']
        ...
        for i, file_path in enumerate(self.val_files):
            ...
            # sub_attributes = np.vstack((data['intensity'], np.zeros_like(data['numberofreturn']), np.zeros_like(data['returnnumber']))).T
            # sub_attributes = np.zeros_like(sub_attributes)
            # sub_attributes = data['intensity']
...
if __name__ == '__main__':
    ...
    if Mode == 'train':
        ...
        # snap_path = '/home/qll3h/ql_project/SQN_ALS_Classification/models/Shatin_xyz_intensity_nr_rn/snapshots'
