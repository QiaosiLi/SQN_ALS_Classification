...
class Shatin:
    def __init__(self):
        ...
        self.input_colors = {'training': [], 'validation': []}  # todo
    ...
    def load_sub_sampled_clouds(self):
        ...
        for i, file_path in enumerate(self.train_files):
            ...
            ann_attributes = np.vstack((normalize([data['intensity']]), data['numberofreturn'], data['returnnumber'])).T
            ann_labels = data['class']
            # data.dtype.names  ('x', 'y', 'z', 'intensity', 'numberofreturn', 'returnnumber', 'class')
            # ann_attributes = normalize([data['intensity']])
        ...
        for i, file_path in enumerate(self.val_files):
            ...
            sub_attributes = np.vstack((normalize([data['intensity']]), data['numberofreturn'], data['returnnumber'])).T
            # sub_attributes = normalize([data['intensity']]).reshape(-1, 1)
...
if __name__ == '__main__':
    ...
    if Mode == 'train':
        ...
        # snap_path = '/home/qll3h/ql_project/SQN_ALS_Classification/models/Shatin_xyz_nor_intensity_nr_rn/snapshots'
