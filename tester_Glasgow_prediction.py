...
class ModelTester:
    def __init__(self, model, dataset, raw_data_path, save_path, restore_snap=None):
        ...
        while last_min < num_votes:
            try:
                ...
                if last_min + 1 < new_min:
                    ...
                    for i_test in range(num_test):
                        out_las_file = join(self.save_path, dataset.input_names['test'][i_test] + '.las')
                        if exists(out_las_file):
                            continue
                        ...
                        # save_name = join(test_path, 'test_preds', dataset.input_names['test'][i_test] + '.npy')
                        save_name = join(self.save_path, dataset.input_names['test'][i_test] + '.npy')
                        preds = preds.astype(np.uint8)
                        preds[preds == 3] = 6 # asprs code: building
                        preds[preds == 2] = 5 # asprs code: vegetation
                        preds[preds == 1] = 2 # asprs code: ground
                        preds[preds == 0] = 1 # asprs code: unclassified
                        # print(preds)
                        # print(np.unique(preds, return_counts=True))
                        np.save(save_name, preds)
                        # read las
                        las_file = join(self.raw_data_path, dataset.input_names['test'][i_test] + '.las')
                        las_data = laspy.read(las_file)
                        las_data.points['classification'] = preds
                        out_las_file = join(self.save_path, dataset.input_names['test'][i_test] + '.las')
                        las_data.write(out_las_file)
                        print(dataset.input_names['test'][i_test] + ' done')
